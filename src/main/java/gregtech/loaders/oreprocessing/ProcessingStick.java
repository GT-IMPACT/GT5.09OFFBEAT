...
public class ProcessingStick implements gregtech.api.interfaces.IOreRecipeRegistrator {
    ...
    public void registerOre(OrePrefixes aPrefix, Materials aMaterial, String aOreDictName, String aModName, ItemStack aStack) {
        ...
        if (!aMaterial.contains(gregtech.api.enums.SubTag.NO_WORKING)) {
            ...
            GT_Values.RA.addLatheRecipe(GT_OreDictUnificator.get(OrePrefixes.stick, aMaterial, 2L), GT_OreDictUnificator.get(OrePrefixes.stickLong, aMaterial, 1L), null, (int) Math.max(aMaterial.getMass() * 3L, 1L), 16);
            GT_Values.RA.addCutterRecipe(GT_Utility.copyAmount(1L), GT_OreDictUnificator.get(OrePrefixes.bolt, aMaterial, 4L), null, (int) Math.max(aMaterial.getMass() * 2L, 1L), 4);
        ...
        if (!aMaterial.contains(gregtech.api.enums.SubTag.NO_SMASHING)) {
            ...
            GT_Values.RA.addForgeHammerRecipe(GT_Utility.copyAmount(2L), GT_OreDictUnificator.get(OrePrefixes.stickLong, aMaterial, 1L), (int) Math.max(aMaterial.getMass(), 1L), 16);
        }
        GT_Values.RA.addAssemblerRecipe(GT_OreDictUnificator.get(OrePrefixes.stick, Materials.Wood, 1), ItemList.Circuit_Integrated.getWithDamage(0L, 2L, new Object[0]), Materials.SeedOil.getFluid(50L), ItemList.FR_Stick.get(1L, new Object[0]), 16, 8);
    }
}
