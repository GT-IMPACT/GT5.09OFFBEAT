...
public class ProcessingStick implements gregtech.api.interfaces.IOreRecipeRegistrator {
    ...
    public void registerOre(OrePrefixes aPrefix, Materials aMaterial, String aOreDictName, String aModName, ItemStack aStack) {
        ...
        if (!aMaterial.contains(gregtech.api.enums.SubTag.NO_WORKING)) {
            GT_Values.RA.addLatheRecipe(aMaterial.contains(SubTag.CRYSTAL) ? GT_OreDictUnificator.get(OrePrefixes.gem, aMaterial, 2L) : GT_OreDictUnificator.get(OrePrefixes.ingot, aMaterial, 2L), GT_OreDictUnificator.get(OrePrefixes.stick, aMaterial, 2L), GT_OreDictUnificator.get(OrePrefixes.dust, aMaterial.mMacerateInto, 1L), (int) Math.max(aMaterial.getMass() * 5L, 1L), 16);
