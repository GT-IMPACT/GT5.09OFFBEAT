...
public class GT_MetaTileEntity_WorldAccelerator extends GT_MetaTileEntity_TieredMachineBlock {
	...
	public ITexture[] getTexture(IGregTechTileEntity pBaseMetaTileEntity, byte pSide, byte pFacing, byte pColorIndex, boolean pActive, boolean pRedstone) {
		if (mMode == 0) {
			return new ITexture[]{Textures.BlockIcons.MACHINE_CASINGS[mTier][pColorIndex + 1], pSide < 2 ? null : pActive ? TextureFactory.of(_mGTIco_Norm_Active) : TextureFactory.of(_mGTIco_Norm_Idle)};
		} else {
			return new ITexture[]{Textures.BlockIcons.MACHINE_CASINGS[mTier][pColorIndex + 1], pSide < 2 ? null : pActive ? TextureFactory.of(_mGTIco_TE_Active) : TextureFactory.of(_mGTIco_TE_Idle)};
		}
	}
	...
	public boolean onWrenchRightClick(byte aSide, byte aWrenchingSide, EntityPlayer pPlayer, float aX, float aY, float aZ) {
		...
		markDirty();
		GT_Utility.sendChatToPlayer(pPlayer, String.format("Machine acceleration changed to x%d", mAccelerateStatic[getSpeedTierOverride()]));
	...
	public void onScrewdriverRightClick(byte pSide, EntityPlayer pPlayer, float pX, float pY, float pZ) {
		if (pPlayer.isSneaking()) {
			if (mMode == 0) {
				...
				markDirty();
				GT_Utility.sendChatToPlayer(pPlayer, String.format("Machine radius changed to %d Blocks", getRadiusTierOverride()));
			} else
				GT_Utility.sendChatToPlayer(pPlayer, String.format("Can't change radius; Machine is in TileEntity Mode!"));
		} else {
			mMode = (byte) (mMode == 0x00 ? 0x01 : 0x00);
			markDirty();
			GT_Utility.sendChatToPlayer(pPlayer, String.format("Switched mode to: %s", mModeStr[mMode]));
		}
	}
	...
	private boolean isTEBlackListed(TileEntity pTile) {
      ...
      // Don't accelerate ANY solars
      return tSimpleClassName.contains("solar") || tCanonicalName.contains("solar");
    }
