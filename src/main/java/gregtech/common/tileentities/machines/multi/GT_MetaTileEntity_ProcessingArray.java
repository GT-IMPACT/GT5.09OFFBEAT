...
public class GT_MetaTileEntity_ProcessingArray extends GT_MetaTileEntity_MultiBlockBase {
    ...
    public String[] getDescription() {
        return new String[]{
                "Controller Block for the Processing Array Mk 1",
    ...
    public ITexture[] getTexture(IGregTechTileEntity aBaseMetaTileEntity, byte aSide, byte aFacing, byte aColorIndex, boolean aActive, boolean aRedstone) {
        if (aSide == aFacing) {
            return new ITexture[]{Textures.BlockIcons.casingTexturePages[0][48], TextureFactory.of(aActive ? Textures.BlockIcons.OVERLAY_FRONT_PROCESSING_ARRAY_ACTIVE : Textures.BlockIcons.OVERLAY_FRONT_PROCESSING_ARRAY)};
        }
    ...
    public GT_Recipe.GT_Recipe_Map getRecipeMap() {
        ...
        } else if (tmp.startsWith("wireassembler")) {
            return GT_Recipe.GT_Recipe_Map.sWireAssemblerRecipes;
        }
        return null;
    }
    ...
    public boolean checkRecipe(ItemStack aStack) {
        ...
        if (tInputList.size() > 0 || tFluids.length > 0) {
            ...
            if (tRecipe != null) {
                ...
                int machines = Math.min(16, mInventory[1].stackSize << mMult); //Upped max Cap to 64
                int i = 0;
                for (; i < machines; i++) {
