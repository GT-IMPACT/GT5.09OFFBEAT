...
public class GT_NEI_SawMill
    ...
    public GT_NEI_SawMill(GT_Recipe.GT_Recipe_Map aRecipeMap) {//this is called when recipes should be shown
        this.mRecipeMap = aRecipeMap;
        this.transferRects.add(new RecipeTransferRect(new Rectangle(59, 28, 32, 15), getOverlayIdentifier(), new Object[0]));
    ...
    public TemplateRecipeHandler newInstance() {
        NEI_GT_Config.SAW=new GT_NEI_SawMill(this.mRecipeMap);
        return NEI_GT_Config.SAW;
    }
        ...
        if (recipeDesc == null) {
            if (tEUt != 0) {
                ...
                if (this.mRecipeMap.mShowVoltageAmperageInNEI) {
                    ...
                    switch (tSpecial) {
                        case 0:
                            drawText(10, 113, "Mode: Planks & Sawdust", 0x602487);
                            break;
                        case 1:
                            drawText(10, 113, "Mode: Wood Pulp & Sawdust", 0x602487);
                            break;
                        case 2:
                            drawText(10, 113, "Mode: Only Sawdust", 0x602487);
                            break;
                        default:
                            drawText(10, 113, "", 0x602487);
                    }
                } else {
            ...
            if (tDuration > 0) {
                drawText(10, 103, trans("158","Time: ")+String.format("%.2f " + trans("161"," secs"), 0.05F * tDuration), -16777216);
            }
            int tSpecial = ((CachedDefaultRecipe) this.arecipes.get(aRecipeIndex)).mRecipe.mSpecialValue;
    ...
    public class CachedDefaultRecipe
        ...
        public CachedDefaultRecipe(GT_Recipe aRecipe) {
            ...
            if (obj != null) {
                this.mInputs.add(new FixedPositionedStack(obj, 23, 18));
            }

            if (aRecipe.mSpecialItems != null) {
                this.mInputs.add(new FixedPositionedStack(aRecipe.mSpecialItems, 23, 18));
            }

            for(int i = 0; i < 4; i++) {
                if (aRecipe.getOutput(i) != null) {
                    this.mOutputs.add(new FixedPositionedStack(aRecipe.getOutput(i), 18 * (i % 2) + 109, 18 * (i / 2) + 18, aRecipe.getOutputChance(i)));
                }
            }

            if ((aRecipe.mFluidInputs.length > 0) && (aRecipe.mFluidInputs[0] != null) && (aRecipe.mFluidInputs[0].getFluid() != null)) {
                this.mInputs.add(new FixedPositionedStack(GT_Utility.getFluidDisplayStack(aRecipe.mFluidInputs[0], true), 23, 36));
            }
